...
// takes the Gallery's photos prop object, width of the container, margin
// between photos Gallery prop, and columns Gallery prop. calculates, sizes
// based on columns and returns the photos object with new height/width props
export function computeSizes({photos, columns, width, margin}) {
    if (!width) {
        return [];
    }
    // divide photos over rows, max cells based on `columns` effectively resulting
    // in [[0, 1, 2], [3, 4, 5], [6, 7]]
    ...
    const rowsWithSizes = rows.map((row, rowIndex) => {
        ...
        // assign height, but let height of a single photo in the last row not expand
        // across columns so divide by columns
    ...
    return rowsWithSizes.reduce((acc, row) => [
        ...acc,
        ...row
    ], []);
}